...
import type { CliConfig, LoadedConfig } from "./types.js";
import { validateTargetDirectory } from "./validation.js";

function parseConfig(content: string, filePath: string): CliConfig {
  let parsed: unknown;
  ...
  try {
    ...
    throw new ConfigError(
      `Failed to parse ${path.basename(filePath)} as JSON.`,
      "Fix the JSON syntax or run `iconium init` to recreate the config.",
  ...
  if (!parsed || typeof parsed !== "object") {
    throw new ConfigError(
      `Invalid config format in ${path.basename(filePath)}.`,
...
export function getConfigPaths(cwd: string): { canonicalPath: string; legacyPath: string } {
  return {
...
export function saveConfig(cwd: string, config: CliConfig): string {
  ...
  try {
    ...
    throw new FileSystemError(
      `Failed to write config file at ${canonicalPath}.`,
